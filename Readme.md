...
| Admin email (subject, HTML, plain text, attachment file name) | Yes |
| API response, success popup, page banner | No |
| Student email (subject, HTML, plain text); no attachment | No |
| Student-audience PDF rendering (not sent to anyone) | No |
...
`services/pdfGenerator.js` renders an A4 document with PDFKit and appends documents with pdf-lib. The queue always generates the **admin** audience; the file is written to the job's upload directory as `admission-<uuid>.pdf` (written once, only after rendering and merging succeed) and is deleted with that directory.
...
The `student` audience omits the Internal Application Information and office sections and footer ID. It exists in code and tests but is not generated by the workflow.
...
| Attachment | Combined admin PDF, `<Application ID>-<Name>-Admission-Form.pdf` | None |
| Internal data | Included | Excluded: built only from applicant-facing fields; a guard refuses to send if the message contains the Application ID, "Application ID", "Office Use", "Admission No", "Verified By", "Remarks", or an attachment |
...
6. Submit a test application with test recipients and confirm: success message without ID, reset form, admin email with PDF, student email without ID or attachment, new row in `Ground School Admissions`, and a reservation in `GroundSchoolApplicationIDs`.
